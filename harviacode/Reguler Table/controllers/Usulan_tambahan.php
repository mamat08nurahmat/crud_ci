...
class Usulan_tambahan extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'usulan_tambahan/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'usulan_tambahan/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'usulan_tambahan/index.html';
            $config['first_url'] = base_url() . 'usulan_tambahan/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Usulan_tambahan_model->total_rows($q);
        $usulan_tambahan = $this->Usulan_tambahan_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'usulan_tambahan_data' => $usulan_tambahan,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->load->view('usulan_tambahan/usulan_tambahan_list', $data);
    }
...
/* Generated by Harviacode Codeigniter CRUD Generator 2017-06-09 04:59:06 */
/* http://harviacode.com */