<!doctype html>
<html>
    <head>
        <title>harviacode.com - codeigniter crud generator</title>
        <link rel="stylesheet" href="<?php echo base_url('assets/bootstrap/css/bootstrap.min.css') ?>"/>
        <style>
            body{
                padding: 15px;
            }
        </style>
    </head>
    <body>
        <h2 style="margin-top:0px">Usulan_tambahan List</h2>
        <div class="row" style="margin-bottom: 10px">
            <div class="col-md-4">
                <?php echo anchor(site_url('usulan_tambahan/create'),'Create', 'class="btn btn-primary"'); ?>
            </div>
            <div class="col-md-4 text-center">
                <div style="margin-top: 8px" id="message">
                    <?php echo $this->session->userdata('message') <> '' ? $this->session->userdata('message') : ''; ?>
                </div>
            </div>
            <div class="col-md-1 text-right">
            </div>
            <div class="col-md-3 text-right">
                <form action="<?php echo site_url('usulan_tambahan/index'); ?>" class="form-inline" method="get">
                    <div class="input-group">
                        <input type="text" class="form-control" name="q" value="<?php echo $q; ?>">
                        <span class="input-group-btn">
                            <?php 
                                if ($q <> '')
                                {
                                    ?>
                                    <a href="<?php echo site_url('usulan_tambahan'); ?>" class="btn btn-default">Reset</a>
                                    <?php
                                }
                            ?>
                          <button class="btn btn-primary" type="submit">Search</button>
                        </span>
                    </div>
                </form>
            </div>
        </div>
        <table class="table table-bordered" style="margin-bottom: 10px">
            <tr>
                <th>No</th>
		<th>Jenis</th>
		<th>Cif</th>
		<th>Jenis Cif</th>
		<th>Nama Nasabah</th>
		<th>Cif Utama</th>
		<th>Hubungan Degan Utama</th>
		<th>Status</th>
		<th>Approval</th>
		<th>Tanggal Kirim</th>
		<th>Action</th>
            </tr><?php
            foreach ($usulan_tambahan_data as $usulan_tambahan)
            {
                ?>
                <tr>
			<td width="80px"><?php echo ++$start ?></td>
			<td><?php echo $usulan_tambahan->jenis ?></td>
			<td><?php echo $usulan_tambahan->cif ?></td>
			<td><?php echo $usulan_tambahan->jenis_cif ?></td>
			<td><?php echo $usulan_tambahan->nama_nasabah ?></td>
			<td><?php echo $usulan_tambahan->cif_utama ?></td>
			<td><?php echo $usulan_tambahan->hubungan_degan_utama ?></td>
			<td><?php echo $usulan_tambahan->status ?></td>
			<td><?php echo $usulan_tambahan->approval ?></td>
			<td><?php echo $usulan_tambahan->tanggal_kirim ?></td>
			<td style="text-align:center" width="200px">
				<?php 
				echo anchor(site_url('usulan_tambahan/read/'.$usulan_tambahan->id),'Read'); 
				echo ' | '; 
				echo anchor(site_url('usulan_tambahan/update/'.$usulan_tambahan->id),'Update'); 
				echo ' | '; 
				echo anchor(site_url('usulan_tambahan/delete/'.$usulan_tambahan->id),'Delete','onclick="javasciprt: return confirm(\'Are You Sure ?\')"'); 
				?>
			</td>
		</tr>
                <?php
            }
            ?>
        </table>
        <div class="row">
            <div class="col-md-6">
                <a href="#" class="btn btn-primary">Total Record : <?php echo $total_rows ?></a>
		<?php echo anchor(site_url('usulan_tambahan/excel'), 'Excel', 'class="btn btn-primary"'); ?>
		<?php echo anchor(site_url('usulan_tambahan/word'), 'Word', 'class="btn btn-primary"'); ?>
	    </div>
            <div class="col-md-6 text-right">
                <?php echo $pagination ?>
            </div>
        </div>
    </body>
</html>