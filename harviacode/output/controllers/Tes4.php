...
class Tes4 extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'tes4/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'tes4/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'tes4/index.html';
            $config['first_url'] = base_url() . 'tes4/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tes4_model->total_rows($q);
        $tes4 = $this->Tes4_model->get_limit_data($config['per_page'], $start, $q);
        ...
        $this->load->view('tes4/tes4_list', $data);
    }
