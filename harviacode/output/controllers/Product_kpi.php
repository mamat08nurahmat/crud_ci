...
class Product_kpi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'product_kpi/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'product_kpi/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'product_kpi/index.html';
            $config['first_url'] = base_url() . 'product_kpi/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Product_kpi_model->total_rows($q);
        $product_kpi = $this->Product_kpi_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'product_kpi_data' => $product_kpi,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->load->view('product_kpi/product_kpi_list', $data);
    }
