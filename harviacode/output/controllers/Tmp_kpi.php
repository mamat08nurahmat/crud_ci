...
class Tmp_kpi extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if ($q <> '') {
            $config['base_url'] = base_url() . 'tmp_kpi/index.html?q=' . urlencode($q);
            $config['first_url'] = base_url() . 'tmp_kpi/index.html?q=' . urlencode($q);
        } else {
            $config['base_url'] = base_url() . 'tmp_kpi/index.html';
            $config['first_url'] = base_url() . 'tmp_kpi/index.html';
        }

        $config['per_page'] = 10;
        $config['page_query_string'] = TRUE;
        $config['total_rows'] = $this->Tmp_kpi_model->total_rows($q);
        $tmp_kpi = $this->Tmp_kpi_model->get_limit_data($config['per_page'], $start, $q);

        $this->load->library('pagination');
        $this->pagination->initialize($config);

        $data = array(
            'tmp_kpi_data' => $tmp_kpi,
            'q' => $q,
            'pagination' => $this->pagination->create_links(),
            'total_rows' => $config['total_rows'],
            'start' => $start,
        );
        $this->load->view('tmp_kpi/tmp_kpi_list', $data);
    }
